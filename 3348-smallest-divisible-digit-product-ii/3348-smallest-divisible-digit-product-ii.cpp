...
public:
    string smallestNumber(string num, long long t) {
        int div[4] = {2,3,5,7}; // every number in the string num or t should consist of only these factors if the factors are not there in t t factors > 2,5,7 then return -1

        // means precheck the value of t if valid then go on if not then return "-1"
        return "-1";
    }
};
